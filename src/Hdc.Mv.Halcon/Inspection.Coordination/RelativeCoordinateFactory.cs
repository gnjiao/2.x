...
namespace Hdc.Mv
{
    public static partial class RelativeCoordinateFactory
    {
...
//            double actualAvgVectorOKAngle;
        ...
        public static IRelativeCoordinate CreateCoordinateUsingBorder(IList<EdgeSearchingResult> results)
        {
            ...
            if (double.IsNaN(vectorCenter.X) || double.IsNaN(vectorCenter.Y))
            {
                return new MockRelativeCoordinate();
            }
        ...
        public static IRelativeCoordinate CreateCoordinateUsingSingleLine(EdgeSearchingResult edgeSearchingResult)
        {
            var vectorCenter = edgeSearchingResult.EdgeLine.GetCenterPoint().ToVector();
            var lineVector = edgeSearchingResult.EdgeLine.GetVectorFrom2To1();
        ...
        public static IRelativeCoordinate CreateCoordinateUsingPointAndAngle(double x, double y, double angle)
        {
            var coord = new RelativeCoordinate(new Vector(x, y), angle);
            return coord;
        }
    }
}