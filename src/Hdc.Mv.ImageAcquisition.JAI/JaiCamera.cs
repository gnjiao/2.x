...
namespace Hdc.Mv.ImageAcquisition
{
    ...
    public class JaiCamera : ICamera
    {
        ...
        public void ChangeExposureTime(double expo)
        {
            if (_myCamera.GetNode("ExposureTime") != null)
            {
                _myCamera.GetNode("ExposureTime").Value = expo;
                _myCamera.GetNode("ExposureTime").ExecuteCommand();
            }
        }
    }
}